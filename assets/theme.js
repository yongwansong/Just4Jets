...
customElements.define('loess-product-forms', class extends HTMLElement {
  constructor() {
    super();
...
window.customElements.define('loess-product-filters', class extends HTMLElement {
  ...
  renderProductGrid(html) {
    ...
    document.getElementById('FilterProductGrids').innerHTML = html.getElementById('FilterProductGrid').innerHTML;
  }
...
window.customElements.define('loess-predictive-search', class extends HTMLElement {
  constructor() {
    super();
    this.input = this.querySelector('input[type="search"]'); // Should probably (eventually) isolate this to its own custom element JS
...
window.customElements.define('loess-scroller', class extends HTMLElement {
  constructor() {
    ...
    this.scroller = this.querySelector('ul'); // Hate to have to do this, but getting a bug in which the scroller jumps
  ...
  async mediaQueryHandler() {
    ...
    await window.customElements.whenDefined('loess-scroller-dots'); // Set an initial state for the dots

    this.dots = this.querySelector('loess-scroller-dots');
    (_this$dots = this.dots) === null || _this$dots === void 0 ? void 0 : _this$dots.reset(); // If breakpoints change, we want to disconnect the previous observer

    if (this.observer) this.observer.disconnect(); // No point in doing anything if not enough items
...
window.customElements.define('loess-scroller-tabs', class extends HTMLElement {
  constructor() {
    ...
    this.buttons.forEach(button => {
      button.addEventListener('click', this.onButtonClick.bind(this)); // button.addEventListener('mouseenter', this.onButtonHover.bind(this));
    });
  ...
  updateButtons(button) {
    ...
    button.setAttribute('aria-expanded', 'true'); // Ensures the offscreen button becomes visible when clicked
...
const LoessProductGallery = class extends HTMLElement {
  constructor() {
    ...
    this.transitioning = false; // If variant present, change slide so that it's showing its image
...
window.customElements.define('loess-popup', class extends ExpandableHTMLElement {
  constructor() {
    super();
    if (window.location.pathname === '/challenge') return; // If in the theme editor, let the lifecycle callbacks show/hide the popup
  ...
  _getWithExpiry(key) {
    ...
    const item = JSON.parse(storedItem); // Compare the expiry time of the item with the current time
...
const ProductRecommendations = class extends HTMLElement {
  constructor() {
    super();
    this.setupIntersectionObserver(this.initProductRecommendations.bind(this));
  }
  ...
  buildQueryString() {
    return `${window.LoessTheme.routes.product_recommendations_url}?section_id=${this.sectionId}&product_id=${this.productId}&limit=${this.limit}`;
  }

  injectHTMLResponse(text) {
    const div = document.createElement('div');
    div.innerHTML = text;
    const productRecommendations = div.querySelector('loess-product-recommendations');
...
Object.assign(ProductRecommendations.prototype, IntersectionObserverMixin);
...
const RecentlyViewedProducts = class extends HTMLElement {
  constructor() {
    super();
    this.productIdSet = this.getProductsFromLocalStorage();
    this.setupIntersectionObserver(this.initRecentlyViewedProducts.bind(this));
  ...
  async initRecentlyViewedProducts() {
    if (this.productIdSet.size === 0) return;
    const response = await fetch(`${this.fetchUrl}&q=${this.buildQueryString()}`);
    const text = await response.text();
    this.injectHTMLResponse(text); // Only able to clear history outside of the theme editor
  ...
  getProductsFromLocalStorage() {
    ...
    const productIdSet = new Set(storedProductIds); // Removes the current viewed product from the set, if it exists

    productIdSet.delete(Number(this.excludedProductId));
    return productIdSet;
  }

  buildQueryString() {
    return [...this.productIdSet].map(productId => `id:${productId}`).join(' OR ');
  }

  injectHTMLResponse(text) {
    const div = document.createElement('div');
    div.innerHTML = text;
    const recentProductsList = div.querySelector('loess-recently-viewed-products');

    if (recentProductsList && recentProductsList.innerHTML.trim().length) {
      this.innerHTML = recentProductsList.innerHTML;
    }
  }
  ...
  get excludedProductId() {
    return this.getAttribute('excluded-product-id');
  }

};
Object.assign(RecentlyViewedProducts.prototype, IntersectionObserverMixin);
window.customElements.define('loess-recently-viewed-products', RecentlyViewedProducts);
...
var input_val = document.querySelector('#FooterNewsletterForm--footer').value.length;
var ghgh = document.querySelector('[action="/contact#ContactFooter"]');
var body = document.querySelector('body');
if (result == true) {
  if (input_val != 0) {
    //body.insertAdjacentHTML("beforebegin", `<div id="close-pop" class='pop-subsc-action'><p>Already Subscribed</p><span id="close-pop-btn">✖</span></div>`);
    ghgh.insertAdjacentHTML("beforebegin", `<div id="close-pop" class='already'>Already Subscribed</div>`);
    console.log("already subscribed");

  }
  else if (input_val == 0) {
    //body.insertAdjacentHTML("beforebegin", `<div class='pop-subsc-action'><p>Please add your email</p><span id="close-pop-btn">✖</span></div>`);
  }
}
else {
  console.log('false');
...
const cat = localStorage.getItem('curSelds');
if (cat == undefined) {
  localStorage.setItem('curSelds', '1');
  document.getElementById("first-banner").style.display = "block";
}
document.getElementById("close-pop-btns").onclick = function () {
